...
<head>
    ...
    <style>
	.scrollable-div li:not(:has(> input[type="checkbox"])) {
		list-style: none;
		background: url("images/gis/i-menuBtn.svg") no-repeat;
			...
			<div class="gis-search">
				<!-- 리스트 -->
				  ...
				  <div class="gis-list-table">
					...
					<div>
					  <button type="submit" id="category-reset">초기화</button>
					</div>
																...
																<c:forEach var="list4" items="${list}">
																  <c:if test="${list4.parent_id == list3.id && list4.level == list3.level + 1}">
																	<c:choose>
																	  ...
																	  <c:otherwise>
																		<li>
																		  <input type="checkbox" class="checkLayer" id="checkLayer${list4.id}">${list4.name}
																		</li>
																	  </c:otherwise>
																	</c:choose>
																  </c:if>
